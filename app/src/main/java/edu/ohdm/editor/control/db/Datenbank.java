...
public class Datenbank extends SQLiteOpenHelper {
    ...
    public Datenbank(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
    ...
    public Eintrag[] getEntries()
    {
        ...
        Eintrag[] entries = new Eintrag[cursor.getCount()];
        cursor.moveToFirst();

        for(int i = 0; i< cursor.getCount();i++)
        {
            entries[i] = new Eintrag(cursor.getString(0),(cursor.getInt(1) == 1));
            cursor.moveToNext();
        }
    ...
    public void saveEntries(Eintrag[] entries)
    {
